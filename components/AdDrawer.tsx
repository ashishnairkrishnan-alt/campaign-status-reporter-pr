  ...
  return (
    <>
      ...
      <aside className="fixed right-0 top-0 h-full w-full max-w-md bg-white border-l border-border z-50 flex flex-col drawer-enter overflow-hidden shadow-2xl">
        {/* Header */}
        <div
          className="flex items-start justify-between p-5 border-b-[3px] border-b flex-shrink-0"
        ...
        >
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg text-subtle hover:text-ink hover:bg-surface transition-colors flex-shrink-0">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="aspect-video bg-surface border-b border-border relative flex items-center justify-center">
            {ad.thumbnailUrl ? (
              ...
              <div className="flex flex-col items-center gap-2 text-subtle">
                {hasVideo ? <Play className="w-10 h-10 text-blue/40" /> : (
          ...
          <div className="p-5">
            ...
            {ALL_METRICS.filter(({ key }) => {
              ...
              <MetricRow key={key} label={label} value={formatMetric(ad.metrics[key] as number, key)} />
            ))}
          </div>
          ...
          <div className="px-5 pb-6 border-t border-border pt-5">
            <div className="flex items-center justify-between mb-3">
              ...
              {suggestion && (
                <button onClick={fetchSuggestion} className="text-[10px] text-blue hover:text-blue-dark flex items-center gap-1">
            ...
            {loading ? (
              <div className="space-y-2">
                {[100, 88, 76].map((w, i) => <div key={i} className="skeleton h-3 rounded-full" style={{ width: `${w}%` }} />)}
              </div>
            ) : suggestion ? (
